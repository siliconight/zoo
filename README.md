...
| Species | An asset type — one per genome under `zoo_keeper/genome/species/`; `python -c "from zoo_keeper.core import genome; print(len(genome.list_species()))"` is the count (a number written here goes stale; this one was 29 while the library held 104) |
...
## Light fixtures (Deli Counter / Lot / Lux integration)

Light in DELCO comes from the sun or from physical fixtures — never from
nowhere. Deli Counter derives WHERE lights belong and emits
`<building>.lights.json`; Lot merges every building's manifest plus its own
exterior streetlight anchors into one site manifest; Lux spawns the actual
Light3D rigs at each anchor. Zoo is the hardware leg: it consumes the **same
manifest** and bakes the visible fixtures the light appears to come from, so
one contract feeds two consumers with zero drift.

Plan (pure, no Blender — prints JSON):

    python tools/zoo_cli.py --fixtures path/to/<building>.lights.json

Build (needs Blender):

    blender --background --python tools/zoo_cli.py -- \
      --fixtures path/to/<building>.lights.json --theme delco --out art_zoo

Exports `<scope>_fixtures.glb` + a `.built.json` index. Works per building
(DC manifest) or on a Lot-merged site manifest; `--fixture-types streetlight`
builds only the exteriors out of a site file whose interiors are already
baked per-building. Placement math mirrors Lux's rigs exactly: rows expand
centered on the anchor along `rot_y`; fluorescent housings hang above the
emitter (filling DC's ceiling gap, diffuser face at the anchor); streetlight
...
**Card shop (5, 0.95.0):** `display_case` (the glass-top showcase counter, forms flat / L -- the L's corner is one case, not two boxes), `pack_wall` (a gondola bay of booster displays under a coloured header sign, on `slatwall`), `pennant_row` (angled felt pennants along a wall top), `folding_table` (forms bare / cloth, dressed with `_surface_stock`'s `cards` flavour) and `folding_chair`. Every brand on them is invented — `core/card_brands.py`, guarded by a denylist test against the real card games, publishers, graders, leagues and clubs. Stock is boxes with a texture and capped by count, never "as many as fit": the whole room (one L case, four pack bays, four pennant rows, three tables, eight chairs) measures **10,664 triangles** at every worst-case genome corner. See the 0.95.0 CHANGELOG entry for the per-species budgets and what each cap bought.

**The till (1, 1.0.0):** `cash_register` -- a 1997 electronic register, and the walker's frame from cold run 9062 is why it exists: what stood on the card shop's counter was four boxes drawn inside `display_case` (`display_case_forms._register`) and read as "a brown box with a white post and a smaller box on top". The species is a squat body on a deeper cash drawer with a full-width pull and a round lock barrel, a raised CUSTOMER display on a short post -- **green type on a black screen**, the one thing the walker named -- a reflective green operator panel tipped toward the clerk, a receipt printer with a curl of paper in it, and a keypad painted into the atlas rather than modelled. **134 triangles** at every one of the 27 genome corners against a 200 budget, and **37 KiB** of decoded art at the default slot (66 at the widest): twenty modelled keys would have been 240 triangles on their own, which is more than the whole register. The display is EMISSIVE AND NOT A LIGHT -- `M_Register_*_Face`, so Lux's binder cuts it with the building's power, and nothing spawns a Light3D in a package that already ships 84. The type is `pixel_type`, the factory's own face, at the largest whole scale the display fits: 35 mm of green at the default slot, which reads at 6 m in a 960 x 640 frame. `display_case` gained `params.till` (default 1, unchanged) so a placer that stands one of these can turn the old boxes off; see the 1.0.0 CHANGELOG entry for what Deli Counter has to do.

**Flat art (4, 0.98.0):** the cheap half of "the card shop is not dense enough" — `poster` (forms `framed` / `bare` / `tilted`: a painted plate with the title across the TOP in a drawn serif, one creature centred and looking out, a warm ground and architecture behind, the maker's mark in the bottom corner; the framed form is three rectangles — frame ring, mat ring, plate — because the mat is what makes it read as framed rather than taped up), `hanging_banner` (printed cloth on a rod under two straps, with a weighted hem), `ceiling_hanger` (a painted board on a drop chain, printed both sides from one tile) and `aisle_sign` (the hand-lettered section sign, words from `card_brands.AISLE_SAYS`). `_surface_stock`'s `cards` flavour also gained a textured path, so a playmat is a printed scene rather than a flat colour with two border slabs. **What it costs:** a saturated room — eight posters, three banners, four hangers, three signs, two tables of mats — is **948 triangles** of flat art against the card shop's measured 10,664, and **2.392 MiB** of decoded art across 18 images. Geometry is free here and the texture is not; `tools/flat_art_cost.py` is the measurement and the 0.98.0 CHANGELOG entry says what a shared cross-module atlas, 512 px/m and a die-cut hanger would each buy. **What a thing hangs from:** the top of the slot box is the ceiling plane — there is no ceiling-grid geometry and the light pipeline's `hang` mount is for lamps, so the hanging point is Deli Counter's `level_design._piece(..., under=...)`, the one `pennant_row` has used since 0.95.0. The two hung species top out at 0.60 m, derived from the contract's 2.0 m headroom at the library's shortest storey.

**Street signs (0.96.0):** `stop_sign` (a 30 in octagon with the white STOP, MUTCD R1-1) and `sign_post`, a 3 lb/ft galvanised u-channel carrying one blade named by the slot's `form` -- `no_parking` (MUTCD R8-3a, the symbolic slashed P), `ped_crossing` (MUTCD W11-2's yellow diamond over the W16-7P arrow plaque) or `bus_stop` (a transit flag, which no part of the MUTCD governs), and the bare pole when a slot names none. Each form's slot is the sign's own size plus its MUTCD 2A.18 mounting height, so the three are different sizes on purpose; `lot/site_furniture.BLADE_DIMS` is the same table on the other side. Collision is the POST only. 272-576 triangles against a 900 budget; see the 0.96.0 CHANGELOG entry.
